...
public class VentaController {
    ...
    public Venta registrar(@Valid @RequestBody VentaRequest request) {
        VentaService.ClienteData client = request.cliente() == null ? null : new VentaService.ClienteData(
            request.cliente().numeroDocumento(), request.cliente().nombresRazonSocial(), request.cliente().direccion(),
        ...
        return service.registrar(request.empresaId(), request.usuarioId(), request.clienteId(), client,
            request.tipoComprobanteId(), request.numeroComprobante(), request.metodoPago(), request.itemVentaList());
    }
    public record VentaRequest(@NotNull(message = "La empresa es obligatoria") Integer empresaId,
        ...
        public boolean tieneCliente() { return clienteId != null || cliente != null; }
    ...
    public record ClienteRequest(@NotBlank(message = "El documento del cliente es obligatorio") @Size(max = 20, message = "El documento no puede superar 20 caracteres") String numeroDocumento,
